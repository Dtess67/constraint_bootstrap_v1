...
class Handle:
    ...
    def update(self, matched: bool, update_truth: bool = False) -> None:
        ...
        if matched:
            ...
            self.misses += 1
            # Both decay on mismatch if we were confident enough to be penalized
...
class BootstrapAgentV1:
    ...
    def predict(self, sent: Seq) -> Decision:
        """
        Predict response using strongest handle that matches exact sent signature.
        Returns a Decision object with lane, act, and optional question.
        
        Routing Rules:
        1. SILENT: Zero matching handles in the registry.
        2. NA: Matching handles exist but are below eligibility_min_to_consider.
        3. QUESTION: 
           - Weak Knowledge: Candidates exist but fail Speak gates.
           - Conflict: Two strong candidates have strengths too close together.
        4. SPEAK: Top candidate passes Speak gates and wins competition.
        """
        sent_s = _sig(sent)
        
        # All potential matches in registry
        all_registry_matches = [h for h in self.handles if h.sent_sig == sent_s]
        if not all_registry_matches:
            return Decision(lane=Lane.SILENT)
        ...
        if not all_candidates:
            return Decision(lane=Lane.NA)

        # Use Strength gate for active candidates
        # Redundancy Check: Since strength = min(eligibility, truth), 
        # strength >= min_strength_to_predict handles the truth gate if min_strength == truth_min_to_speak.
        # We use min_strength_to_predict as the master gate.
        ...
        # If we have matches but NONE are strong enough, ask a question.
        if not active_candidates:
            ...
            return Decision(lane=Lane.QUESTION, question=question, meta={"reason": "weak_knowledge", "top1": h1.hid, "eligibility": h1.eligibility, "truth": h1.truth})

        # Middle Lane for conflicting strong knowledge
        # If we have strong candidates but they are too close in strength, ask a question.
        if len(active_candidates) >= 2:
            margin = active_candidates[0].strength - active_candidates[1].strength
            # Use conflict_margin for the middle lane.
            if margin < self.conflict_margin:
                ...
                return Decision(lane=Lane.QUESTION, question=question, meta={"reason": "conflict", "margin": margin, "top1": h1.hid, "top2": h2.hid, "s1": h1.strength, "s2": h2.strength})

        h = active_candidates[0]
        act = tuple(int(x) for x in h.resp_sig.split(",")) if h.resp_sig != "0" else ()
        return Decision(lane=Lane.SPEAK, act=act, meta={"hid": h.hid, "strength": h.strength, "eligibility": h.eligibility, "truth": h.truth})
    ...
    def observe(self, sent: Seq, received: Seq, learn: bool = True, update_truth: bool = True) -> StepMetrics:
        """
        ...
        if not learn:
            ...
            if self._seen_counts[key] >= self.promote_threshold:
                if not any(h.sent_sig == sent_s and h.resp_sig == recv_s for h in self._handles):
                    self._total_handles_created += 1
                    hid = f"H{self._total_handles_created:03d}"
                    # New handles born with 0.25 eligibility and 0.35 truth (initial speakable truth)
                    self._handles.append(Handle(hid=hid, sent_sig=sent_s, resp_sig=recv_s, truth=0.35))
        ...
        for h_cand in allowed:
            if h_cand.sent_sig == sent_s and h_cand.resp_sig == recv_s:
                # If we are in QUESTION/SILENT/NA lane, we ONLY want to update eligibility.
                # h_cand.update(matched=True) updates eligibility.
                # update_truth controls if truth is also updated.
                h_cand.update(matched=True, update_truth=update_truth)
                updated_any = True
                winner = h_cand
            elif h_cand.sent_sig == sent_s and h_cand.resp_sig != recv_s:
                # Competing response for same sent -> penalize
                # Note: we update truth if requested. 
                # If we are NOT updating truth, mismatches on other lanes shouldn't penalize truth?
                # Actually, mismatch usually means this handle is NOT the correct one for this input.
                # But if we are in QUESTION lane, we are just exploring.
        ...
        if self.silence_penalty > 0.0 and is_truly_silent and received:
            ...
            for h_s in self._handles:
                if h_s.sent_sig == sent_s and h_s.resp_sig == recv_s:
                    ...
                    if update_truth:
                        h_s.truth = min(1.0, h_s.truth + self.silence_penalty)
